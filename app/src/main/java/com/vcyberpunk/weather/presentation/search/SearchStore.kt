...
import com.vcyberpunk.weather.domain.usecase.ChangeFavouriteStateUseCase
import com.vcyberpunk.weather.domain.usecase.SearchCityUseCase
import com.vcyberpunk.weather.presentation.search.SearchStore.Intent
...
class SearchStoreFactory @Inject constructor(
    ...
    private sealed interface Msg {
        ...
        data object EmptySearchResult : Msg

        data class SuccessSearchResult(val cities: List<City>) : Msg
                ...
                Intent.ClickSearch -> {
                    ...
                    searchJob = scope.launch {
                        ...
                        try {
                            ...
                            if (result.isEmpty()) {
                                dispatch(Msg.EmptySearchResult)
                            } else {
                                dispatch(Msg.SuccessSearchResult(result))
                            }
                        } catch (_: Exception) {
    ...
    private object ReducerImpl : Reducer<State, Msg> {
        override fun State.reduce(msg: Msg): State {
            return when (msg) {
                ...
                Msg.EmptySearchResult -> {
                    copy(
                        searchState = State.SearchState.EmptyResult
                    )
                }
                ...
                is Msg.SuccessSearchResult -> {
                    copy(
                        searchState = State.SearchState.SuccessLoaded(msg.cities)
                    )
                }
            }
        }
